...
import CameraCaptureBanner from "@/features/fly/components/CameraCaptureBanner.jsx"
...
const pipClass = getPipClass(false)
...
const FlyPage = () => {
  ...
  return (
    <div className="relative h-full min-h-0 w-full overflow-hidden bg-[#06090E] select-none">
      ...
      <div
        id="drone-live-feed"
        className={mapIsLarge ? getPipClass(camSelected === "fpv") : fullClass}
      ...
      {/* Docked Camera Controls when Camera is Full Screen */}
      {!mapIsLarge && (
      ...
      <div className="absolute top-1.5 sm:top-3 left-1/2 -translate-x-1/2 z-15 pointer-events-auto flex items-center gap-1.5 sm:gap-2.5 px-2 sm:px-3 py-0.5 sm:py-1 rounded-md bg-[#080C14CC] border border-[#1A2633] backdrop-blur-md shadow-lg text-[8.5px] sm:text-[10px] font-mono max-w-[calc(100vw-180px)] sm:max-w-none overflow-hidden">
